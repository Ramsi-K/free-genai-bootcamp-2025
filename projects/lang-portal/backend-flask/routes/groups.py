...
def load(app):
    ...
    def get_group_words(id):
        try:
            ...
            sort_by = request.args.get('sort_by', 'hangul')  # ✅ Fixed sorting default
            order = request.args.get('order', 'asc')

            # Validate sort parameters
            valid_columns = ['hangul', 'romanization', 'english', 'type', 'correct_count', 'wrong_count']
            if sort_by not in valid_columns:
                sort_by = 'hangul'
            if order not in ['asc', 'desc']:
                order = 'asc'
            ...
            cursor.execute(f'''
                SELECT w.id, w.hangul, w.romanization, w.english, w.type,
                    w.example_korean, w.example_english,
    ...
    def get_group_study_sessions(id):
        try:
            ...
            # Get study sessions for this group
            cursor.execute(f'''
                SELECT s.id, s.group_id, s.study_activity_id, s.created_at as start_time,
                    (
            ...
            return jsonify({'study_sessions': cursor.fetchall()})
        except Exception as e:
            return jsonify({"error": str(e)}), 500
