...
def concatenate_images_grid(images, dist_images, output_size):
    ...
    for index, img in enumerate(images):
        ...
        x_offset = (
            col * (cell_width + dist_images) + (cell_width - new_width) // 2
        )
        y_offset = (
            row * (cell_height + dist_images) + (cell_height - new_height) // 2
        )
...
def enable_caching(model_id, cache_dir):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-path", type=str, default="llava-hf/llava-1.5-7b-hf"
    )
