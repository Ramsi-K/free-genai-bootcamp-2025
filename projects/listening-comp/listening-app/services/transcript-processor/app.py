...
def get_transcript(video_id):
    ...
    try:
        ...
        logger.error(
            f"Error fetching Korean transcript for video {video_id}: {e}"
        )
        return None, []
...
def get_transcript_by_video_id(video_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT start, end, duration, text FROM segments WHERE video_id = ?",
            (video_id,),
        )
        segments = [
            {
        ...
        logger.error(
            f"Database error while fetching transcript for video {video_id}: {e}"
        )
        return jsonify({"error": "Database error"}), 500
    except Exception as e:
        logger.error(
            f"Unexpected error while fetching transcript for video {video_id}: {e}",
            exc_info=True,
        )
        return jsonify({"error": "Internal server error"}), 500
